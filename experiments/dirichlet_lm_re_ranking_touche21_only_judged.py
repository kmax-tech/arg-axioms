...
if __name__ == '__main__':
    ...
    experiment_name = 'DirichletLM-Reranking-Only-Judged'
    print(s.dataset_index_dir)

    indexref = pt.IndexRef.of(str(s.dataset_index_dir))
    index = pt.IndexFactory.of(indexref)
    stat = index.getCollectionStatistics()

    rerank_nbr = 5 # number of documents to rerank
    ...
    res_df = rd.adjust_drop_missing_docnos(res_df) # cut df to human judgments only
    ...
    metrics = [ir_measures.nDCG(judged_only=True)@5,ir_measures.nDCG()@5]
    metric_names = ['nDCG(judged_only=True)@5','nDCG@5']

    axioms = arg_axiom_list + old_axioms
    axioms_names = arg_axiom_name_list + old_axioms_names

    experiment = re_rank_argu_axioms(experiment_name,
                                     res_df,
                                     axioms=axioms,
    ...
    plt.savefig(f'f{experiment_name}.pdf' , format='pdf' , bbox_inches='tight')
    plt.show()
